...
class ISLES_TrainDataset(Dataset):

    def __init__(self, img_paths, mask_paths, x_transform=None, y_transform=None):
        self.img_paths = img_paths
        self.mask_paths = mask_paths
    ...
    def __getitem__(self, idx):
        ...
        if self.x_transform is not None:
            image = self.x_transform(npimage)

        if self.y_transform is not None:
            mask = self.y_transform(nplabel)

        return image, mask, np_fn